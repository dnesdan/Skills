...
| Imagegen UI Prototypes | `prototype-ui-with-imagegen` | Native visual divergence and promotion | Uses Codex's built-in image generation to explore and compare distinct Apple or Android UI directions without changing code, then optionally rebuilds an explicitly selected direction in SwiftUI or Compose and validates it at runtime. |
...
- Image-generated UI concepts stay outside production during exploration, are judged against native and accessibility constraints, and become real UI only through a selected SwiftUI or Compose rebuild.
